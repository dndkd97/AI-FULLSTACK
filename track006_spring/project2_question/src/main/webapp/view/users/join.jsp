...
<div class="container my-5">
  ...
  <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>          	
    ...
    <div class="my-3  alert  alert-warning  temail"> 
     이메일 중복검사는 필수입니다.
    </div>        
 		 <script>
         window.addEventListener("load",function(){
        	 let email = document.getElementById("email");
        	 let target = document.querySelector(".temail");
        	 ...
        	 if(value!==""){
        		 ...
        		 .then(data=>{
        			 if(data.exists){
        				 ...
        				 target.className="my-3 alert alert-danger temail";
        			 } else{
        				 target.textContent="사용가능한 이메일 입니다";
        				 target.className="my-3 alert alert-info temail";
        			 } }).catch(err=>{
        			 target.textContent="서버오류 입니다";
    				 target.className="my-3 alert alert-danger temail";
        		 });
