...
select  d.DEPTNO ,DNAME ,e.EMPNO ,ENAME , SAL
from emp e ,dept d
...
order by d.DEPTNO asc ,empno asc;
...
select*from emp where sal< any(select max(sal) from emp where deptno=30 )order by sal;
select*from emp;
select*from emp where deptno=30; -- 2850
...
select*from emp where deptno=30; -- 950
select*from emp;


-- 연습문제-14
select*from emp where sal < any(select min(sal) from emp where deptno=30)order by sal;
...
-- 연습문제 - 19
select empno, ename , deptno , dname , loc from emp natural join dept where deptno=10;
select*from dept;
...
from emp e join dept d
on e.deptno = d.deptno where job=(select job from emp where ename='allen');
...
join salgrade s
on e.sal between s.losal and s.hisal
where sal >(select avg(sal) from emp) order by sal desc ,e.empno asc;
...
from emp e join salgrade s
on e.sal between s.losal and s.hisal
where sal > any(select MAX(sal) from emp where job like 'SALESMAN');
select*from emp;
