...
from group_userinfo group by ban;

select ban from group_userinfo;
...
-- +------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select ban, sum(kor)as 국어총합,sum(eng)as `영어총합`, sum(math)as '수학총합' from group_userinfo group by ban;
--  
...
-- +------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select ban, avg(kor)as `국어평균`, avg(eng)as `영어평균`, avg(math) as `수학평균` from group_userinfo group by ban;
--  
...
-- +------+----------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select ban, sum(age) as `나이총합` , avg(age) as `나이평균`, max(age) as `최고나이` , min(age) as `최저나이` from group_userinfo group by ban;
--  
...
select ban ,avg(kor) as `국어평균` ,avg(eng) as `영어평균` , avg(math) as `수학평균` 
from group_userinfo group by ban 
having avg(kor)>=89;
--  
...
-- +------+----------+----------+----------+----------+
-- 2 rows in set (0.00 sec)
select ban , sum(age) as `나이총합` , avg(age) as `나이평균` , max(age) as `최고나이` , min(age) as `최저나이`
from group_userinfo
group by ban
having avg(age>=35);
--  
...
-- 3 rows in set (0.00 sec)
select*from group_userinfo;
select ban, sum(age<20) as`미성년자수` 
from group_userinfo
group by ban;
...
-- | B    | 178.3333  |
-- +------+-----------+
select ban, sum(kor+eng+math)/3
from group_userinfo
group by ban
having sum(kor+eng+math)/3>=80;

select ban, ((sum(kor)/3)+(sum(eng)/3)+(sum(math)/3))
from group_userinfo
group by ban
having ((sum(kor)/3)+(sum(eng)/3)+(sum(math)/3))>=80;
...
select ban, (avg(kor)+avg(eng)+avg(math))/3
from group_userinfo
group by ban
limit 1;
...
select sum(comm) as `추가수당` from emp;
-- --------------------------------------------------------
-- -------------------------------------------------------- 
-- -- 연습문제-3
...
-- mysql>
select*from emp;
select count(*) as `부서 30 직원수` from emp where dept=30; -- 집계전 필터링
select count(*) `부서 30 직원수` from emp group by dept having dept=30; -- 집계후 필터링
...
-- 1 row in set (0.00 sec)
select*from emp;
select count(*) as `추가수당을 받는 사원수` from emp where comm>=0;
...
select count(*) as `추가수당을 받는 사원수` from emp where comm is not null;
--  
...
--  
select*from emp;
select max(sal) as `부서10 최대급여` from emp where dept=10;
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 연습문제-10
...
--    
select min(sal) as `부서10 최소급여` from emp where dept=10;
...
select*from emp;
-- mysql>
select max(hiredate) as `부서20 최근 입사일` from emp where dept=20;
...
-- +---------------------------+
-- 1 row in set (0.00 sec)
select min(hiredate) as `부서20 제일오래 된 입사일` from emp where dept=20;
--  
...
select avg(sal) as `부서30 평균급여` from emp where dept=30;
--  
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 연습문제-14
...
select avg(distinct sal) as `부서30 중복제거 급여평균` from emp where dept=30;
--  
...
-- +--------+-----------+
-- 3 rows in set (0.00 sec)
select dept , avg(sal) as `평균급여` from emp group by dept order by avg(sal)desc; 
--  
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 연습문제-16
...
select dept, job, avg(sal) as `평균급여` from emp group by dept,job order by dept asc;
...
select dept,job,avg(sal) as `평균급여` from emp
group by dept,job
having avg(sal)>=2000
order by dept asc;
--  
...
-- where에서 집계조건 사용 불가 having에서 사용 가능 having avg(sal) >=2000
--   
...
-- +--------+-----------+--------+----------+--------+-----------+
-- 9 rows in set (0.00 sec)
select  dept,job,count(*) as`사원수`, max(sal) as `최고급여` , sum(sal) as `급여합`, 
