...
select*from join_userban where no=6 and ban="C";
...
select a.no, a.name , a.age , b.ban
from join_userinfo a join join_userban b
on a.no = b.no where a.age>20 and a.age<40; 
-- >>> (3) join  using
select a.no, a.name , a.age , b.ban
from join_userinfo a join join_userban b
using(no) where a.age>20 and a.age<40; 
-- >>> (4) natural join
select no,name,age,ban
from join_userinfo natural join join_userban where age>20 and age<40;
...
select a.no, a.name,b.ban,a.age
from join_userinfo a left join join_userban b 
on a.no=b.no;
...
-- -- 이코드가 의미하는바는?
-- -- 조인조건이 없으면 두 테이블 모두 조회 emp 14줄, dept 4줄 카다시안 곱 - 모든조합
...
from emp E join dept D on E.deptno=D.deptno where E.sal>=3000;
...
from emp E ,SALGRADE S where E.sal>S.losal and E.sal<S.hisal order by losal;
...
select e.empno , e.ename  ,e.mgr  , s.empno`MGR_EMPNO` , s.ename`MGR_ENAME`
from emp e, emp s , salgrade m where e.sal between m.losal and m.hisal and e.mgr=s.empno order by e.mgr asc;
...
from emp e , emp m
where e.mgr=m.empno order by e.mgr , e.ename asc
;
...
on e.mgr=m.empno order by e.empno;
select*from emp;
-- mysql>
...
on e.mgr=m.empno order by e.empno asc;
select*from emp;
...
from emp natural join dept order by dname;
select*from emp;
...
using(deptno)
where e.sal>=3000 order by deptno;
select*from dept;
...
-- +--------+------------+-----------+---------+---------+-----+
-- 3 rows in set (0.00 sec)
select d.DEPTNO , d.DNAME  ,avg(e.sal)`AVG_SAL` , max(e.sal)`MAX_SAL` ,min(e.sal) `MIN_SAL` ,count(*) `CNT` 
from  emp e,dept d
