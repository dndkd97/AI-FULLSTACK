...
delete from select_userinfo where no in(9,10,12,13); -- delete from select_userinfo where no>=5; -- 5번이상 데이터 삭제
-- mysql>
...
-- +----------+--------+-----------+
-- 14 rows in set (0.00 sec)
select  empno as `사원번호` , ename as `이름` , job `담당 업무` from emp;
...
-- mysql>
 ...
 select * from emp;
 select empno as `EMPLOYEE_NO` ,ename as `EMPLOYEE_NAME` ,job as `JOB`, mgr as `MANAGER` , hiredate as `HIREDATE` , 
 sal as `SALARY` , comm as `COMMISSION` , dept as `DEPARTMENT_NO` from emp order by dept desc, ename asc;
desc milk;
select*from userinfo;
select*from milk;
desc milk;
use mbasic;
alter table milk modify mname varchar(50) null;
alter table milk modify mprice int null;
alter table milk modify mno int null;

delete from milk where no=2;
...
ALTER TABLE milk
RENAME COLUMN mtotla TO mtotal;
-- <!--       주문현황표             -->
-- <!--       주문현황표             -->
-- <!-- 
-- = MODEL
...
update milk_order set oname='choco' and onum=2 where onum=1;
-- -- Q5.  milk_order 해당번호의 데이터 삭제
delete from milk_order where onum=2;
