...
create table userinfo_select select*from userinfo where 1=2; -- 구조복사alter
-- select*from userinfo where 1=2; 1=2같니? 해당하는 값 x , 구조만 복사
...
insert into userinfo_select (no,name,age) values(4,'frouth',44);
select*from userinfo_select;
...
select*from userinfo_select where age is not null and no between 2 and 3  and name not like'_e%';
...
select*from emp;
select dept , job , count(*) as `사원수` , max(sal) as `최고급여` , sum(sal) as `급여합`, avg(sal) as `평균급여` from emp group by dept,job order by dept asc;
-- >> 주어진조건
-- mysql> desc emp;
-- +----------+-------------+------+-----+---------+----------------+
-- | Field    | Type        | Null | Key | Default | Extra          |
...
-- = BASIC 
select ceil(1.1) `올림2`, floor(1.9) `내림1` , round(1.6) `반올림2` ,mod(10,3) `나머지` from dual;
select 12.2345 
...
select 123.4578 , round(123.4578, 0) `ROUND1` ,  round(123.4578,-1) `ROUND2`
                , round(123.4578,1) `ROUND3`, round(123.4578,2) `ROUND4` from dual;
...
-- +----------+------+-------+
-- 1 row in set (0.00 sec)
select 123.4578 , ceil(123.4578)`CEIL` , floor(123.4578)`FLOOR` from dual;
...
-- +----------+-----------+-----------+
-- 1 row in set (0.00 sec)
select instr('abc','b')`a(1)b(2)c에서 b-2`, instr('abc','ab')`ab의 위치` , instr('abc','ac')`ac의 위치(없으면 0)` from dual;
...
-- +------+------+------+
-- 1 row in set (0.00 sec)
select substr('abcde',2,2)`s1` , substr('abcde',2,3)`s2`, substr('abcde',1,3)`s3` from dual;
...
-- +--------------------------+
-- 1 row in set (0.00 sec)
select concat('choco','mlik') from dual;
-- mysql>
-- mysql> -- 7. 공백빼기 - trim 
-- +-----------------+
-- | trim(' a b c ') |
...
-- +---------+----------+----------+
-- 1 row in set (0.00 sec)
select concat('#' ,  trim(' a b c ')  , '#' ), -- trim 양쪽공백빼기
	   concat('#' , rtrim(' a b c ')  , '#' ), -- ltrim 왼쪽공백빼기
       concat('#' , ltrim(' a b c ')  , '#' )  -- rtrim 오른쪽공백빼기
        from dual;
-- mysql>
...
alter table fn_select_userinfo2 modify email varchar(20) not null;
...
insert into fn_select_userinfo2 values(4, 'aaa' , 44 , 'ddd@gmail.com');
...
update fn_select_userinfo2 set name='ddd' where age=44;
-- mysql>
...
-- +------+-------------+------------+
-- 6 rows in set (0.00 sec)
select name , left(name,1)`첫번째 글자` ,right(name,1)`마지막글자` from fn_select_userinfo2;
...
select name , replace(name, 'aaa', 'aaa1등') from fn_select_userinfo2;
-- mysql>
-- mysql>
...
-- +------+------+
-- 6 rows in set (0.00 sec)
select name, concat(left(name,1) , '*' , right(name,1))  from fn_select_userinfo2;
-- mysql>
...
-- +-------+-------+
select*from select_userinfo2;
-- select name, replace(name, substr(3)) `test` from select_userinfo2;
...
select round(avg(age),1)`평균나이` from select_userinfo2;
...
-- +--------+
-- select from select_userinfo2;
select count(*) as `인원수`  from select_userinfo2 where age>=40;
...
select max(age)`최대나이`  from select_userinfo2 where email like '%gmail.com%';
...
-- |    11     |
-- +-----------+
select min(age) `최소나이` from select_userinfo2 where length(name);
...
-- |   49.0    |
-- +-----------+
select round(avg(age),1) `평균나이` from select_userinfo2 where age>=30; -- 49.5
...
-- |     3     |
-- +-----------+
select count(*) `b포함인원` from select_userinfo2 where name like '%b%';
...
select name,age from select_userinfo2 order by age desc limit 1;
