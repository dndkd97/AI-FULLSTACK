...
public class ArrayListVsVector {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread t1 = new Thread(()->{
			for(int i=0; i<100; i++) {arrayList.add(i); try {
				Thread.sleep(100);
		...
		Thread t2 = new Thread(()->{
			for(int i=100; i<200; i++) {arrayList.add(i); try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}}
		...
		Thread t3 = new Thread(()->{
			for(int i=0; i<100; i++) {vector.add(i); try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}}
		...
		Thread t4 = new Thread(()->{
			for(int i=100; i<200; i++) {vector.add(i); try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}}
