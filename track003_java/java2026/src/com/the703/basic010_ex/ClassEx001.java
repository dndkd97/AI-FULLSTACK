...
//2. 상태(멤버변수)와 행위(멤버함수)
//3. 설계도 → 부품객체 → 사용
class Student001{ //6번째 줄 ~ 13번째 줄 : 설계도
	String name; 
	int no, kor ,eng , math;
	
	void info() {
	int total=kor+eng+math;
	double evg=(kor+eng+math)/3.0;
	System.out.printf("이름:%s\n총점:%d\n평균:%.2f",name,total,evg );}}
/*
 System.out.println("이름 : " + name );
 System.out.println("총점 : " + (kor+eng+math));
 System.out.printf("평균 : %2.f" , (kor+eng+math)/3.0);
*/	
public class ClassEx001 {
	public static void main(String[] args) {
		  Student001   s1 = new Student001(); // 1) new 번지, 객체생성 2) 생성자-초기화 3) s1주소 갖게해줌
