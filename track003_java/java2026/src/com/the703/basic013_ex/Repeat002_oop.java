...
class Account {
    private int balance;              //인스턴스변수-this 각각 - heap area - new - 생성자
    public static int accountCount=0; //클래스변수-공유 - method area - new X( 생성자랑 관련 X )
    public final int id;              //인스턴스변수-this 각각 - heap area - new - 생성자
...
:-의미:모든 객체에서 공유
 ...
 -적용:AccountCount.accountCount
Q4. final 키워드의 의미를 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.
:-의미:변경 불가 / int id:한번초기화되면 변경불가 , this.id=++accountCount;초기화
Q5. Account.accountCount의 값은 얼마인가? 왜 그렇게 되는지 설명하시오.
:2개, 클래스변수
 Account 객체가 2개 / 생성될때마다 기본생성자가 호출되어 ++accountCount; 실행
...
:-static 메서드는 객체 생성 없이 호출가능,클래스에 속함
 -인스턴스 메서드는 객체 생성 후 호출,객체에 속함
Q9. final 키워드가 변수, 메서드, 클래스에 각각 적용될 때 의미를 설명하시오.
:변수값 변경불가, 메서드 오버라이딩 불가, 클래스 상속불가
Q10. 캡슐화의 장점은 무엇인가?
:-데이터보호(외부 직접 접근 차단)
