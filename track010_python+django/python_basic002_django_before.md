...
```bash
# pip : Node.js의 npm, Java Gradle/Maven 같은 패키지 설치 관리
...
# 답: 1) (비즈니스 로직처리) / 2) (db와 template 연결)
...
## M(model) T(html,css:화면) V(Controller:비지니스로직, db와 view 연결)
...
urlpatterns = [
    ...
    # htpp://127.0.0.1:8000 접속시, /dashboard/로 자동이동
