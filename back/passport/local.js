...
const { findUserEmail } = require('../models/users');  //db사용자 조회함수 
module.exports = () => {
  passport.use(new LocalStrategy(
           {  usernameField: 'email',    // 로그인시 사용할 필드명 req.body.emial
              passwordField: 'password', // 로그인시 사용할 필드명 req.body.password
           }, //설정
           async (email, password, done) => {
                try{
                    const user = await findUserEmail(email);  // db에서 이메일로 사용자 조회
                    if (!user) {
