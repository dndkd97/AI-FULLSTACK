 ...
 * passport/index.js
 * -------------------
 * 로그인: [local.js] Passport LocalStrategy로 인증 후 세션(serializeUser)
...
module.exports = () => { 
    ...
    passport.serializeUser((user, done) => { // 로그인 인증된 사용자 객체정보, 콜백(어떻게 처리됐는지) 
        done(null, user.APP_USER_ID);  // 에러,세션에 저장할 번호
     });
    // 세션에 저장된 사용자 id로 db에서 사용자 조회
    passport.deserializeUser(async (id, done) => { //id,콜백(어떻게 처리됐는지)
        try {
