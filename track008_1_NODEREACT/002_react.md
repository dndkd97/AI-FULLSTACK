...
2. 설정파일
front/
├── .babelrc                # Babel 설정 파일 : jsx → 일반 js로 변환 브라우저 이해할 수 있게, 최신문법(async/await,                 optional chaining 등)
├── .eslintrc               # ESLint 설정 파일 : 일관된 코드스타일 유지(들여쓰기,세미콜론,따옴) / 팀규칙
└── setupTests.js           # 테스트 환경 설정 파일 : 테스트환경 설정파일 환경초기화
...
5. 개발 (reducer - sage - view) (1) reducer
...
5. taskLatest - 특정액션을 감지하고 가장 마지막 액션만 처리
