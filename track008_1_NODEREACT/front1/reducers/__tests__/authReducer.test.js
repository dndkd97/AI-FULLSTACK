...
describe('user slice reducer', ()=>{
    ...
    it('signupSuccess',()=>{ 
        // signupSuccess: (state,action)=>{} / {type:signupSuccess, payload:userData}
    ...
    it('resetUserState' , ()=>{      
        ...
        //2. 리듀서툴킷에서 {type:fetchUserSuccess, payload:userData} 객체만들기
        //3. 리듀서의 fetchUserSuccess: (state,action)=>{} 액션받아서 처리 - 상태초기화
        expect(state.loading).toBe(false); // state.user = action.payload
