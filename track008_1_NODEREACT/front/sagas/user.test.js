...
describe('user saga',()=>{
    ...
    it('deleteUser success',()=>{
        const action = {type:DELETE_USERS_REQUEST,data:{id:1}};
        const gen = deleteUser(action);

        expect(gen.next().value).toEqual(call(deleteUserApi,action.data.id));
        expect(gen.next().value).toEqual(put({type:DELETE_USERS_SUCCESS,data:{id:1}}));
        
        const error ={response:{data:'삭제 실패'}};
        expect(gen.throw(error).value).toEqual(put({type:DELETE_USERS_FAILURE,error:'삭제 실패'}));
    });
});