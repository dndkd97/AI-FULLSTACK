...
public class AppUserServiceImpl implements AppUserService{
	...
	@Transactional	//sql 두개이상 쓸 때
	@Override
	public int insert(MultipartFile file, AppUserDto dto) { //회원가입
		dto.setUfile("the703.png");
		if(!file.isEmpty()) {
			try { dto.setUfile(upload.fileUplad(file)); } 
	...
	public int update(MultipartFile file, AppUserDto dto) {
		...
		// 이미지 업로드 기능
		// dto.setUfile(dto.getUfile()); //기존 이미지 (생략 가능)
		if(!file.isEmpty()) {
			try { dto.setUfile(upload.fileUplad(file)); } 
			catch (IOException e) {  e.printStackTrace(); }
		}
	...
	@Override
	public AppUserAuthDto readAuthByEmail(String email, String provider) { //로그인
	...
	@Override
	public AppUserDto selectEmail(String email, String provider) { //마이페이지
		AppUserDto dto = new AppUserDto(); dto.setEmail(email);
		return dao.findByEmail(dto);
	}

	@Override
	public int iddouble(String email, String provider) { //이메일중복
		AppUserDto dto = new AppUserDto(); dto.setEmail(email);
		return dao.iddoubleByEmail(dto);
	}

	@Override
	public boolean matchesPassword(String email, String provider, String rawPassword) { //로그인한 유저맞는지 확인 - 비밀번호 맞는지 확인
		// 1. dbUser 찾기
		...
		return result!= null && result.getPassword()!= null &&
				passwordencoder.matches(rawPassword, result.getPassword()); //사용자가 입력한 비번과 db비번 일치 확인
	}
