...
import com.thejoa703.repository.DeptUserRepositroy;
...
class Boot2ApplicationTests_1_Entity {
	...
	// insert : save / select : findBy / updatd: save / delete : delete
	   @BeforeEach
	   void setup() {
	      //사용자 생성
	   ...
	   void testAppUserRepository() {
		   ...
		   System.out.println("이메일 중복검사 성공");
	   ...
	   // CommentRepositroy
	   @Test
	   @DisplayName("■ CommentRepositry-CRUD")
	   void testCommentRepositry() {
		   //댓글생성
		   Comment comment = new Comment();
		   comment.setContent("테스크 댓글");
	   ...
	   //PostLikeRepositroy
	   @Test
	   @DisplayName("PostLikeRepositroy-crud")
	   void testPostLikeRepositroy() {
		   //좋아요생성
	   ...
	   @Autowired private DeptUserRepositroy deptUserRepositroy;
	   ...
	   void dept(){
		   DeptUser dept1 = new DeptUser();
		      ...
		      deptUserRepositroy.save(dept1);
		      deptUserRepositroy.save(dept2); entityManager.flush(); entityManager.clear();
