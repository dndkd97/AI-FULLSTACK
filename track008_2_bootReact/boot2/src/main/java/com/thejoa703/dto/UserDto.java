...
public class UserDto {
	...
	public static class UserResponseDto{
		...
		private String email;
		private String nickname;
		private String mobile; // 나중에 확장 용도
		private Integer mbtitype; // 나중에 확장 용도
		private String role;
		...
		public static UserResponseDto fromEntity(AppUser user) { // repository 처리해준값
			return UserResponseDto.builder().id(user.getId()).email(user.getEmail()).nickname(user.getNickname())
											.provider(user.getProvider()).role(user.getRole()).createdAt(user.getCreatedAt())
											.ufile(user.getUfile()).build();
		}
