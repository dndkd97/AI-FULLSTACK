...
public class UserService {
	private final AppUserRepository appUserRepository; // ##  @Autowired 대신에 @RequiredArgsConstructor + private final
	...
	public UserResponseDto createUser(UserRequestDto request,MultipartFile profileImage) {
		...
		if(appUserRepository.existsByNickname(request.getNickname())) {
			throw new ResourceNotFoundException("이미 사용중인 사용자입니다.");
		}
...
//		return new UserResponseDto(savedUser);
	...
	public UserResponseDto login(LoginRequest request) {
		AppUser user = appUserRepository.findByEmailAndProvider(request.getEmail(), request.getProvider()!=null? 
					   request.getProvider():"local").orElseThrow(()->new ResourceNotFoundException("사용자를 찾을수 없습니다."));
		//                         사용자가 입력한 값        / db의 비밀번호
...
//	userSerivce.findByEmailAndProvider(userInfo.getEmail(),userInfo.getProvider())
//	.orElseGet(()->userService.saveSocialUser())
   ...
   public String findRoleByUserId(Long userId) {
	...
	public UserResponseDto getUser(Long userId) { // Optional - 값 1개 , null
		AppUser user =  appUserRepository.findById(userId).orElseThrow(()->new ResourceNotFoundException("존재하지 않는 사용자입니다.id"+userId));
		return  UserResponseDto.fromEntity(user);
	}
	//Read : 전체 사용자 수
   ...
   // Update : 프로필 이미지변경
    ...
    public UserResponseDto updateProfileImage(Long userId, MultipartFile profileImage) {
        // 해당유저 번호 받아서 유저찾기
        AppUser user = appUserRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("사용자 찾을수 없습니다.id:"+userId));
        // 수정
