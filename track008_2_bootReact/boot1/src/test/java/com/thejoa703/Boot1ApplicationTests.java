...
class Boot1ApplicationTests {
	...
	void createTest() {
		...
		void testRead() {
			...
			Optional<AppUser> foundEmail = appUserRepository.findByEmail("1@1");
			assertThat(foundEmail).isPresent();
			assertThat(foundEmail.get().getEmail()).isEqualTo("1@1");
		...
		@Test
		@DisplayName("4. DELETE 생성테스트 (save)")
		void testDelete() {
