...
const api = axios.create({
  ...
  // refreshToken이 HttpOnly 쿠키에 저장이 되어있으면 자동으로 포함  
  withCredentials: true,  
  headers: {
    "Content-Type": "application/json",   // 요청기본 josn
...
api.interceptors.request.use(
  (config) => {
    if (typeof window !== "undefined") { // CSR 환경에서만 localStoratge 접근
      ...
      if (accessToken) {
        config.headers.Authorization = `Bearer ${accessToken}`;  // Authoriztion 헤더에 추가
      }
    }
    return config;  
  },
